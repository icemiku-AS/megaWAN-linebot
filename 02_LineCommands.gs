...
function parseCommand(text) {
  ...
  if (!userPrompt) {
    ...
    if (mode === 'program_topic_analysis') userPrompt = '請根據近期使用者聊天、網址快讀、畫重點與封存記憶分析節目話題。';
    if (mode === 'integrate_topics') userPrompt = '請統整近期使用者聊天、網址快讀、畫重點與封存記憶。';
    if (mode === 'weekly_news') userPrompt = '請整理最近 7 天 NewsInbox 中的新聞素材。';
    if (mode === 'manual_news_supplement') userPrompt = '請補充新聞素材；如果要寫入素材池，需要附上網址。';
    if (mode === 'chat') userPrompt = '請簡短介紹你可以協助的事情。';
  }
...
function normalizeHashCommand_(text) {
...
function getHelpTextByCommand_(text) {
  const normalized = normalizeHashCommand_(text);
  ...
  if (normalized === '#help 全部') return [getHelpText(), '', getHelpCleanupText_(), '', getHelpAdminText_(), '', getHelpDataText_()].join('\n');
  return null;
}

function getHelpText() {
  return ['小浣是你的節目素材秘書。', '', '常用功能：', '直接貼網址：收進 NewsInbox。', '#本週新聞：查看最近 7 天新聞素材。', '#新聞補充 文字 + 網址：手動補新聞素材。', '#懶人包 網址：產生網址快讀摘要。', '#節目話題分析：分析近期可聊主題。', '#統整話題：整理近期話題地圖。', '#畫重點 內容：釘選重要想法。', '#封存本週話題：壓縮成 WeeklySummary。', '', '其他說明：#help 清理、#help 管理、#help 資料、#help 全部、#版本'].join('\n');
}

function getHelpCleanupText_() {
  return ['資料維護指令：', '#清空紀錄：清 ConversationLog。', '#清空重點：清 TopicHighlights。', '#清空快讀：清 WebSummary 與 WebTaskQueue。', '#清空封存：清 WeeklySummary。', '#清空新聞：清 NewsInbox 與 NewsUrlQueue。', '#清空待回覆：清 PendingReplies。', '', '所有項目只處理目前聊天室，需再輸入「指令 確認」。'].join('\n');
}

function getHelpAdminText_() {
  return ['管理指令：', '#版本：查看目前版本。', '#版本紀錄：查看近期版本紀錄。', '#reset：清除短期記憶，不動 Google Sheet。', '#help 資料：查看資料表用途。'].join('\n');
}

function getHelpDataText_() {
  return ['主要資料表：', 'ConversationLog：原始對話。', 'TopicHighlights：#畫重點 的人工重點。', 'NewsInbox：新聞素材池。', 'NewsUrlQueue：新聞網址佇列。', 'WebSummary：網址快讀摘要。', 'WebTaskQueue：網址快讀 / 分析任務。', 'WeeklySummary：長期封存記憶。', 'PendingReplies：待交付回覆。'].join('\n');
}
