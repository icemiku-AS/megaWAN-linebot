...
- X / Facebook / Threads 這類目前未支援平台會在 NewsInbox 入隊前直接攔截。
- 這些未支援平台不再被寫入 NewsUrlQueue，也不會重試三次才 failed。
- 混合貼多個網址時，可支援的網址仍會入隊，不支援的社群網址會在回覆中提醒。
- NewsUrlQueue 背景處理遇到 unsupported_social_platform / unsafe_url 這類永久錯誤時會直接 failed，不再重試三次。
