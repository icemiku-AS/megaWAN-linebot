...
- 新增 17_SocialReaderLayer.gs，承接 v1.10.9 社群網址 reader 分流。
- X / Twitter 單篇 status 貼文會透過 FxTwitter API 讀取。
- Facebook、fb.watch、Threads.com、Threads.net 不再於入隊前被視為未支援平台，改先交給 Jina Reader 嘗試讀取。
- 成功讀到的社群內容會整理成 Reader Layer 統一 webResult 格式，後續 NewsInbox、#懶人包、#節目話題分析 沿用既有流程。
- FxTwitter API endpoint 集中放在 00_Config.gs。
- 本版不導入 Apify / ByCrawl，不導入 Node.js / npm，不修改 Google Sheet schema，也不重構 NewsInbox。
...
v1.10.5 起，這個背景流程會先透過 Reader Layer 讀取網頁內容。v1.10.6 起，PTT 文章頁會套用更嚴格的 over18 gate 判斷，避免正常文章被誤判。v1.10.9 起，X / Twitter 單篇 status 會走 FxTwitter API；Facebook、fb.watch、Threads.com、Threads.net 會先走 Jina Reader。
...
- 16_ReaderLayer.gs：Jina Reader、PTT over18、legacy fallback wrapper 與 reader 統一資料契約。
- 17_SocialReaderLayer.gs：v1.10.9 社群網址 reader 分流，包含 X / Twitter FxTwitter API 與 Facebook / Threads Jina route 調整。
- 18_VersionTexts.gs：v1.10.9 的 #版本顯示文字。
- 19_UnsupportedUrlTextV1109.gs：v1.10.9 的非 status 社群網址提示文字。
...
- #版本
- 直接貼一篇 X / Twitter 單篇 status 貼文，確認可進 NewsUrlQueue 並寫入 NewsInbox。
- #懶人包 加上一篇 X / Twitter 單篇 status 貼文，確認可產生 WebSummary / PendingReplies。
- #節目話題分析 加上一篇 X / Twitter 單篇 status 貼文，確認 DeepSeek 可收到 Reader Layer 文字。
- 直接貼 X / Twitter 個人頁，確認非 status 網址不會無效重試。
