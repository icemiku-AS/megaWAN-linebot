...
v1.10.6 修正 PTT 正常文章頁被誤判成滿 18 歲確認頁的問題，避免文章頁只因含有 ask/over18 字樣就被判定讀取失敗。
...
- 新增 18_PttOver18DetectionHotfix.gs，修正 PTT over18 gate 偵測邏輯。
- 正常 PTT 文章頁若已出現 main-content 或 article-meta 結構，就不再判定為滿 18 歲確認頁。
- 不再把 ask/over18 字樣單獨視為滿 18 歲確認頁，避免正常文章頁被誤殺。
- 保留 v1.10.5 Reader Layer 架構：一般網站走 Jina Reader，PTT 走 GAS UrlFetchApp + over18=1 cookie。
...
v1.10.5 起，這個背景流程會先透過 Reader Layer 讀取網頁內容。v1.10.6 起，PTT 文章頁會套用更嚴格的 over18 gate 判斷，避免正常文章被誤判。
...
- PTT：使用 GAS 原生 UrlFetchApp，並帶 over18=1 cookie；v1.10.6 起修正正常文章頁被 over18 gate detector 誤判的問題。
- X、Twitter、Facebook、Threads：本版只偵測為未支援，未來可再接 Apify。
...
- 16_ReaderLayer.gs：Jina Reader、PTT over18、社群平台未支援偵測與 reader 統一資料契約。
- 17_ReaderLayerCompat.gs：Reader Layer 與舊版 06_WebReader.gs 的 legacy fallback 相容層。
- 18_PttOver18DetectionHotfix.gs：修正 PTT over18 gate detector 誤判正常文章頁的 hotfix。
...
- #懶人包 一般新聞網址
- #節目話題分析 一般新聞網址
- PTT 成人看板文章網址，確認不再誤判為滿 18 歲確認頁。
- X、Facebook、Threads 網址，確認本版回報尚未支援。
- #本週新聞
- #help
- #help 資料
- #help 清理
