...
- 新增 16_ReaderLayer.gs，集中管理網址讀取 provider。
- 新增 17_ReaderLayerCompat.gs，讓新 Reader Layer 可安全復用既有 legacy 流程。
- 一般網站優先使用 Jina Reader，將網頁轉成較適合 LLM 使用的文字內容。
- PTT 使用 GAS 原生 UrlFetchApp，並在 request header 帶 Cookie: over18=1 處理滿 18 歲確認頁。
- #懶人包 先透過 Reader Layer 取得正文，再交給 Gemini 做快讀摘要。
- #節目話題分析 加網址時，先透過 Reader Layer 取得正文，再交給 DeepSeek 做節目分析。
- NewsInbox 自動網址分類改用 Reader Layer 回傳的 mainText，再交給 Gemini 分類。
...
v1.10.5 起，這個背景流程會先透過 Reader Layer 讀取網頁內容。
...
- PTT：使用 GAS 原生 UrlFetchApp，並帶 over18=1 cookie。
