...
- `#本週新聞` 預設顯示最近 7 天 NewsInbox 新聞素材。
- `#本週新聞 高潛力` 只顯示 `TopicPotential=高` 的素材。
- `#本週新聞 詳細` 顯示較完整內容大綱、切角與節目潛力。
- `#本週新聞 精簡` 只列分類、標題與來源。
- `#本週新聞 24小時` 只看最近一天素材。
- `#本週新聞 分類 <分類名>` 只看指定分類。
- `#封存本週新聞` 的 prompt 改為「本週新聞週報索引」取向，保留代表性事件、人物、公司、平台、政策、作品名稱與主要脈絡。
- `#help` 只顯示核心新聞工作流；`#help 進階` 收納較少用的新聞檢視與節目整理功能。
...
將最近 7 天 NewsInbox 素材整理成 WeeklySummary，作為新聞長期記憶。這類封存會寫入 `ArchiveType=news`，供未來 `#本週新聞` 比對過去脈絡。v1.12.1 起，封存 prompt 更像「本週新聞週報索引」，會優先保留代表性事件、人物、公司、平台、政策、作品名稱與主要脈絡。
...
查看較少用的新聞檢視與節目整理功能，例如 `#本週新聞 詳細 / 精簡 / 24小時 / 分類`、`#懶人包`、`#節目話題分析`、`#統整話題`、`#畫重點`、`#封存本週話題`。
