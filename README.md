...
- 更新 `12_ResponseTexts.gs` 的目前版本文字與內建版本摘要。
- `#版本紀錄` 只顯示最近 6 筆，避免回覆隨版本增加而過長。
- 完整版本歷史仍以 `99_changelog.md` 為準。
- 本版不修改 Reader Layer、NewsInbox、WebTaskQueue、LINE webhook 主流程或 Google Sheet schema。
...
將 `00_Config.gs` 與 `12_ResponseTexts.gs` 手動同步至 Apps Script 後，在 LINE 測試：
...
確認版本文字為 v1.10.10，版本紀錄只顯示 v1.10.10 至 v1.10.5 共 6 筆，且結尾仍提醒完整歷史以 99_changelog.md 為準。
