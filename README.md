...
- 新增 `#新聞問答 <問題>`，直接根據最近 7 天 NewsInbox 素材回答新聞問題。
- `#新聞問答` 可搭配 `高潛力` 與 `分類 <分類名>` 篩選素材；回答必須附完整原文網址，方便回頭查原文。
- `#本週新聞 精簡` 維持按分類分組，但來源改為完整原文網址，不再只顯示網域。
- `#本週新聞 診斷` 的來源也改為完整原文網址，方便檢查分類問題後直接點回原文。
- 移除 `#本週新聞 24小時` 與 `#本週新聞 24小時 診斷`，避免保留低頻且容易混淆的新聞檢視模式。
- 核心 `#help` 新增 `#新聞問答 <問題>`；`#help 進階` 移除 24 小時說明，保留詳細、精簡、分類與診斷模式。
- 本版不修改 Reader Layer、NewsInbox schema、WeeklySummary schema、群組貼網址靜默收件流程、WebTaskQueue、`#懶人包` 或網址版 `#節目話題分析`。
...
- NewsInbox：新聞素材池；Brief 供 LINE 與 `#本週新聞` 快速瀏覽，Outline 供 `#統整話題` 深度統整，SpecialTopic / CategoryReason / CategoryConfidence / MatchedEntities / ClassificationWarning 供分類稽核、診斷與 `#新聞問答` 使用。
