...
目前版本：**v1.15.0 Unified Research & Capability Edition**，以 v1.14.4 為 baseline。Git 版本不代表 GAS 已部署；現行契約與部署清單見 [CURRENT_VERSION.md](CURRENT_VERSION.md)。

## 現在能做什麼

- 收集群組新聞網址，整理分類、簡介、大綱與故事線，製作本週編輯台。
- 一般聊天可搜尋最新資訊，也可查目前聊天室的近期新聞、人工重點與封存記憶；研究工具全部只讀。
...
普通聊天由模型判斷是否需要搜尋；「幫我查／搜尋／上網查」會強制要求。工具只查有限的近期視窗，沒有結果不等於完整歷史不存在。來源依實際搜尋或工具取得的公開網址列出，最多三筆；來源 bubble 與主回答分開。

### 圖片

私訊直接傳 JPEG/PNG，或用 LINE「回覆」引用圖片後輸入「這是什麼？」。群組請引用圖片後輸入 `#小浣 <問題>`，舊 `#小浣 看圖 <問題>` 仍可使用。

普通圖片問題使用簡單分析；「幫我查最新進度／查證／來源」才要求圖片加搜尋。提及之前收過的素材或人工重點，可加入只讀工具。單純要求圖片重點或問「這是真的嗎？」不強制開昂貴研究；未查證的推測須清楚標示。
