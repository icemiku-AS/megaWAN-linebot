...
- Gemini 只呼叫一次，同時產生 LINE 回覆用的 20 字內 Brief、NewsInbox 保存用的 100～200 字 Outline，以及標題、分類、切角與節目潛力。
- 同步成功後直接寫入 NewsInbox，LINE 只回覆 20 字內 Brief。
...
直接貼上一個可支援的網址時，小浣會透過 Reader Layer 讀取網頁，交給 Gemini 同時產生 20 字內 Brief、100～200 字 Outline 與 NewsInbox 分類資料。成功後會立即回覆短 Brief，並將完整 Outline 與分類資料寫入 NewsInbox。
...
查看最近 7 天收集到的 NewsInbox 新聞素材。每篇會顯示標題、20 字內 Brief、來源網址與節目潛力，不顯示完整 Outline 或切角。
...
本版會在 NewsInbox 最右側新增 `Outline` 欄位。`ensureNewsInboxSheet_()` 會在第一次執行相關流程時自動補上欄位，不需要 migration，也不強制手動執行 setup。
...
確認同步成功時 LINE 只回覆 20 字內 Brief，NewsInbox 新增一筆 `SourceMode=auto_url_sync` 且 Outline 有內容的資料，並且不會同時新增 NewsUrlQueue 或 PendingReplies。
