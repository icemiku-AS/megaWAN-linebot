...
- Gemini 只呼叫一次，同時產生 LINE 回覆用大綱，以及 NewsInbox 的標題、分類、50 字內簡介、切角與節目潛力。
- 同步成功後直接寫入 NewsInbox，LINE 只回覆 100～200 字內容大綱。
...
直接貼上一個可支援的網址時，小浣會透過 Reader Layer 讀取網頁，交給 Gemini 同時產生 100～200 字內容大綱與 NewsInbox 分類資料。成功後會立即回覆大綱，並將素材寫入 NewsInbox。
...
確認同步成功時 LINE 只回覆 100～200 字大綱，NewsInbox 新增一筆 `SourceMode=auto_url_sync` 的資料，且不會同時新增 NewsUrlQueue 或 PendingReplies。
