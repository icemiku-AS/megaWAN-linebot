...
目前 v1.14.1 有 21 個 active runtime source，依領域區段排列：
...
除非維護者明確要求，否則 Codex 不要自行 commit、push、merge 或開 PR。

不要假設 GitHub Desktop 已安裝或仍在使用。本專案目前採用以下本機開發與版本管理流程：

1. 維護者以 VS Code 開啟本機 repository。
2. 維護者先從最新 main 建立非 main 的工作 branch；不要直接修改 main。
3. Codex 在目前本機 branch 中分析與修改檔案。
4. 維護者以 VS Code Source Control 檢查 diff、stage 與 commit；必要時可搭配 `git diff` 檢查。
5. 維護者以 VS Code 內建 Git 或終端機 push。
6. PR 可透過 GitHub 網頁，或 VS Code 的 GitHub Pull Requests and Issues 擴充套件建立與檢查。
7. merge 完成後，維護者切回 main、pull 最新內容，並清理已完成的工作 branch。
8. 維護者手動同步與部署至 Google Apps Script。

GitHub 只作為版本管理來源。push 或 merge GitHub 不代表會自動部署到 GAS；正式部署仍由維護者手動處理。
...
本專案目前改由本機 Codex / VS Code Source Control / git diff / Pull Request 流程協作，因此不需要刻意避開正常的檔案修改，也不需要為了降低 payload 而強行使用小檔案 overlay。
...
* 修改完成後必須讓 VS Code Source Control / git diff / PR diff 能清楚看出改了哪些檔案、為什麼改、是否影響 runtime 行為。
...
如果使用者透過 VS Code Source Control、VS Code 內建 Git 或終端機手動 commit / push / PR，請只整理本機修改結果，不要假設 PR 已建立。
