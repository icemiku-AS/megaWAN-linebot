...
Current Working Version: `v1.12.0 Silent URL Status & News Archive Edition`
Current Working Branch: `feature/v1200-silent-url-status-news-archive`
Target Branch: `main`
Source of Truth before merge: GitHub branch `feature/v1200-silent-url-status-news-archive`
Source of Truth after merge: GitHub `main` branch latest commit
Stable Baseline: `v1.11.2 Brief Range Hotfix` on `main`

v1.12.0 目前是 Silent URL Status & News Archive Edition 的開發狀態。合併前以 `feature/v1200-silent-url-status-news-archive` 的實際檔案為準；合併後應以 `main` 最新 commit 為準。
...
以下檔案代表 v1.12.0 Silent URL Status & News Archive Edition 沿用的 GAS 程式結構：
...
## Explicitly Not Included in v1.12.0
...
## Suggested Smoke Tests for v1.12.0 Runtime

本版會在 WeeklySummary 最右側追加 `ArchiveType`、`PeriodStart`、`PeriodEnd`、`SourceItemCount` 欄位。這是相容式 schema 變更，不重排舊欄位。同步 `.gs` 後建議手動執行一次 `setupLogSheet()` 或任一會觸發 `ensureWeeklySummarySheet_()` 的指令，確認表頭已補齊。
...
* 執行 `#本週新聞`，確認顯示 Brief，且不顯示切角與節目潛力。
* 執行 `#封存本週新聞`，確認 WeeklySummary 新增 `ArchiveType=news` 的新聞封存。
* 再執行 `#本週新聞`，確認若有新聞封存，會嘗試補充過去脈絡。
...
Last Confirmed Version: `v1.12.0 Silent URL Status & News Archive Edition`
Last Confirmed Working Branch: `feature/v1200-silent-url-status-news-archive`
Stable Baseline: `v1.11.2 Brief Range Hotfix` on `main`
Last Confirmed Date: `2026-06-17`
Last Documentation Note: group direct URLs are silent queued intake, #狀態回報 and #封存本週新聞 are available, and WeeklySummary adds archive metadata columns for topic/news memory.
