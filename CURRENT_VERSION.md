...
2. 新增 `17_ReaderLayerCompat.gs`，讓 Reader Layer 可安全復用既有 legacy raw HTML + Gemini extractor 流程。
3. 一般網站優先使用 Jina Reader，將網頁轉成較適合 LLM 使用的文字內容。
4. PTT 使用 GAS 原生 `UrlFetchApp`，並在 request header 帶 `Cookie: over18=1` 處理滿 18 歲確認頁。
5. `#懶人包` 的網址快讀改先透過 Reader Layer 取得正文，再交給 Gemini 做快讀摘要。
6. `#節目話題分析 + 網址` 改透過 Reader Layer 取得正文，再交給 DeepSeek 做節目分析。
7. NewsInbox 自動網址分類改透過 Reader Layer 取得 `mainText`，再交給 Gemini 分類。
8. 舊 raw HTML + Gemini extractor 流程保留為 legacy fallback，不在本版刪除。
