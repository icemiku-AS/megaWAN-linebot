...
Version represented by this Git ref: `v1.12.3 News QA Edition`
Previous stable baseline described in this file: `v1.12.2 News Classification Audit Edition`
...
以下檔案代表 v1.12.3 News QA Edition 沿用的 GAS 程式結構：
...
## Explicitly Not Included in v1.12.3
...
* Reader Layer 大規模重構
* NewsInbox schema 調整、既有欄位重排、改名或 migration
...
## Suggested Smoke Tests for v1.12.3 Runtime

本版不新增或修改 NewsInbox / WeeklySummary 欄位。若部署環境尚未套用 v1.12.2，仍需確認 NewsInbox 已具備 `SpecialTopic`、`CategoryReason`、`CategoryConfidence`、`MatchedEntities`、`ClassificationWarning`；若部署環境尚未套用 v1.12.0，仍需先確認 WeeklySummary 已具備 `ArchiveType`、`PeriodStart`、`PeriodEnd`、`SourceItemCount` 相容欄位。
...
* 執行 `#本週新聞 精簡`，確認按分類分組，且每則只列標題與完整原文網址。
...
* 再執行 `#本週新聞`，確認若有新聞封存，預設或 `#本週新聞 詳細` 會嘗試補充過去脈絡；高潛力、分類與精簡模式只顯示當次查詢結果。
...
Last Confirmed Version at this Git ref: `v1.12.3 News QA Edition`
Previous stable baseline described in this file: `v1.12.2 News Classification Audit Edition`
Last Confirmed Date: `2026-06-24`
Last Documentation Note: `#新聞問答` answers from recent NewsInbox material with full original URLs, `#本週新聞 精簡` keeps category grouping but shows full original URLs, and 24-hour weekly-news views are no longer part of the current command surface.
