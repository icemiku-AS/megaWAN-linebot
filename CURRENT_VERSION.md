...
Current Working Version: `v1.11.2 Brief Range Hotfix`
Current Working Branch: `hotfix/v1112-brief-range`
Target Branch: `main`
Source of Truth before merge: GitHub branch `hotfix/v1112-brief-range`
Source of Truth after merge: GitHub `main` branch latest commit
Stable Baseline: `v1.11.1 Compact News Brief Edition` on `main`

v1.11.2 目前是 Brief 長度策略 hotfix 的開發狀態。合併前以 `hotfix/v1112-brief-range` 的實際檔案為準；合併後應以 `main` 最新 commit 為準。
...
以下檔案代表 v1.11.2 Brief Range Hotfix 沿用的 GAS 程式結構：
...
## Explicitly Not Included in v1.11.2
...
## Suggested Smoke Tests for v1.11.2 Runtime

本版不修改 Sheet schema，不需要 migration，也不強制重新執行 setup。
...
* 確認 Brief 超過 50 字一點時不會被正常硬裁成半句。
* 確認 NewsInbox 同一筆資料仍保存 100～200 字 Outline。
...
同步成功時應回覆自然短 Brief，NewsInbox 新增一筆 `SourceMode=auto_url_sync` 且 Outline 有內容的資料，並且同一網址不會同時新增 NewsUrlQueue 或 PendingReplies。
...
Last Confirmed Version: `v1.11.2 Brief Range Hotfix`
Last Confirmed Working Branch: `hotfix/v1112-brief-range`
Stable Baseline: `v1.11.1 Compact News Brief Edition` on `main`
Last Confirmed Date: `2026-06-16`
Last Documentation Note: direct URL Brief now targets 30-50 characters, allows shorter source-limited text, and only uses a hard cap as a runaway-output guard.
