...
本版修正 v1.10.5 Reader Layer 中 PTT 成人確認頁判斷過寬的問題，並補上 NewsInbox 自動分類需要的 Gemini JSON helper。本版不導入新 provider、不修改資料清理層。
...
5. NewsInbox 自動分類流程會呼叫 `callGeminiJson_()`，但 v1.10.5 main 中沒有實作該 helper，導致背景任務出現 `classification_error` 與 `callGeminiJson_ is not defined`。

主要調整：

1. 將 v1.10.5 原本拆出的 legacy fallback wrapper 與 v1.10.6 PTT detector hotfix 整合回 `16_ReaderLayer.gs`。
2. 刪除過渡用的 `17_ReaderLayerCompat.gs` 與 `18_PttOver18DetectionHotfix.gs`，避免檔案過度分散。
...
6. 在 `08_GeminiService.gs` 新增 `callGeminiJson_()`，提供 NewsInbox 自動分類這類 prompt-to-JSON 任務使用。
...
- `#版本`
- 直接貼可正常讀取的一般新聞網址，確認進 NewsUrlQueue 並可寫入 NewsInbox，且不再出現 `callGeminiJson_ is not defined`。
- `#懶人包 一般新聞網址`
- `#節目話題分析 一般新聞網址`
