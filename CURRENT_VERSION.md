...
Current Working Version: `v1.10.10 Version History Maintenance Edition`
Current Working Branch: `feature/v1110-version-history`
Target Branch: `main`
Source of Truth before merge: GitHub branch `feature/v1110-version-history`
Source of Truth after merge: GitHub `main` branch latest commit
Stable Baseline: `v1.10.9 Social Reader Edition` on `main`

v1.10.10 目前是小型維護版開發狀態。合併前以 `feature/v1110-version-history` 的實際檔案為準；合併後應以 `main` 最新 commit 為準。
...
以下檔案代表 v1.10.10 Version History Maintenance Edition 沿用的 GAS 程式結構：
...
## Explicitly Not Included in v1.10.10
...
* Reader Layer、NewsInbox、WebTaskQueue 行為調整
* LINE webhook 主流程或指令 router 調整
* 新增 LINE 指令
...
## Suggested Smoke Tests for v1.10.10 Runtime
...
確認 `#版本紀錄` 只顯示 v1.10.10 至 v1.10.5 共 6 筆，並在結尾保留完整歷史以 `99_changelog.md` 為準的提醒。

本版修改了 `12_ResponseTexts.gs`，因此需要由維護者手動同步至 Google Apps Script；不需要重新執行既有 setup 函式。

---

## Last Confirmed

Last Confirmed Version: `v1.10.10 Version History Maintenance Edition`
Last Confirmed Working Branch: `feature/v1110-version-history`
Stable Baseline: `v1.10.9 Social Reader Edition` on `main`
Last Confirmed Date: `2026-06-14`
Last Documentation Note: `#版本紀錄` is limited to the latest 6 entries; full history remains in `99_changelog.md`.
