...
Current Working Version: `v1.11.0 Direct URL Summary Edition`
Current Working Branch: `feature/v1111-direct-url-summary`
Target Branch: `main`
Source of Truth before merge: GitHub branch `feature/v1111-direct-url-summary`
Source of Truth after merge: GitHub `main` branch latest commit
Stable Baseline: `v1.10.10 Version History Maintenance Edition` on `main`

v1.11.0 目前是直接網址同步大綱功能的開發狀態。合併前以 `feature/v1111-direct-url-summary` 的實際檔案為準；合併後應以 `main` 最新 commit 為準。
...
以下檔案代表 v1.11.0 Direct URL Summary Edition 沿用的 GAS 程式結構：
...
## Explicitly Not Included in v1.11.0
...
## Suggested Smoke Tests for v1.11.0 Runtime

本版沒有 Sheet schema 變更，不需要執行 setup 或 migration 函式。
...
* 回歸 `#懶人包`、網址版 `#節目話題分析`、`#本週新聞`、`#版本`、`#版本紀錄`。

同步成功時應只回覆 100～200 字內容大綱，NewsInbox 新增一筆 `SourceMode=auto_url_sync` 的資料，且同一網址不會同時新增 NewsUrlQueue 或 PendingReplies。
...
Last Confirmed Version: `v1.11.0 Direct URL Summary Edition`
Last Confirmed Working Branch: `feature/v1111-direct-url-summary`
Stable Baseline: `v1.10.10 Version History Maintenance Edition` on `main`
Last Confirmed Date: `2026-06-14`
Last Documentation Note: single direct URLs synchronously return an outline and write NewsInbox; multi-URL or failed cases fall back to NewsUrlQueue.
