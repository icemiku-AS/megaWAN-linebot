...
Current Working Version: `v1.11.1 Compact News Brief Edition`
Current Working Branch: `feature/v1111-compact-news-brief`
Target Branch: `main`
Source of Truth before merge: GitHub branch `feature/v1111-compact-news-brief`
Source of Truth after merge: GitHub `main` branch latest commit
Stable Baseline: `v1.11.0 Direct URL Summary Edition` on `main`

v1.11.1 目前是短 Brief 與 NewsInbox Outline 功能的開發狀態。合併前以 `feature/v1111-compact-news-brief` 的實際檔案為準；合併後應以 `main` 最新 commit 為準。
...
1. 合併前以目前 working branch 的實際 `.gs` 程式碼為準；合併後以 GitHub `main` branch 最新 commit 為準
...
以下檔案代表 v1.11.1 Compact News Brief Edition 沿用的 GAS 程式結構：
...
## Explicitly Not Included in v1.11.1
...
## Suggested Smoke Tests for v1.11.1 Runtime

本版會在 NewsInbox 最右側新增 `Outline` 欄位。`ensureNewsInboxSheet_()` 會自動補欄，不需要 migration，也不強制重新執行 setup。

將本版修改的 `.gs` 檔手動同步至 Apps Script 後，在 LINE 測試：

* 群組直接貼一個一般新聞網址。
* 個人聊天室直接貼一個一般新聞網址。
* 確認 LINE 只回覆 20 字內 Brief，NewsInbox 同一筆資料保存 100～200 字 Outline。
* PTT 文章、X / Twitter 單篇 status、Facebook / Threads 公開網址。
* 一次貼兩個以上網址，確認改走 NewsUrlQueue。
* Reader 失敗、登入牆、同步 API 失敗或處理過慢時，確認有 queue fallback 或明確錯誤。
* 已有 Pending Reply 時再貼新網址，確認先交付舊結果，新網址進背景 queue。
* 執行 `#本週新聞`，確認只顯示短 Brief，不顯示切角。
...
同步成功時應只回覆 20 字內 Brief，NewsInbox 新增一筆 `SourceMode=auto_url_sync` 且 Outline 有內容的資料，並且同一網址不會同時新增 NewsUrlQueue 或 PendingReplies。

本版修改了 `.gs` runtime，因此需要由維護者手動同步至 Google Apps Script；不需要重新執行既有 setup 函式。

---

## Last Confirmed

Last Confirmed Version: `v1.11.1 Compact News Brief Edition`
Last Confirmed Working Branch: `feature/v1111-compact-news-brief`
Stable Baseline: `v1.11.0 Direct URL Summary Edition` on `main`
Last Confirmed Date: `2026-06-15`
Last Documentation Note: direct URLs return a short Brief, store a full Outline in NewsInbox, and expose that Outline to `#統整話題`.
