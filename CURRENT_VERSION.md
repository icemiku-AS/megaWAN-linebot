...
Version represented by this Git ref: `v1.12.1 Weekly News Query & Help Focus Edition`
Previous stable baseline described in this file: `v1.12.0 Silent URL Status & News Archive Edition`
...
以下檔案代表 v1.12.1 Weekly News Query & Help Focus Edition 沿用的 GAS 程式結構：
...
## v1.12.1 Version Boundary

v1.12.1 是 Weekly News Query & Help Focus Edition。

本版讓 `#本週新聞` 成為新聞素材主入口，並讓 `#help` 聚焦核心功能：

1. `#本週新聞` 預設顯示最近 7 天 NewsInbox 新聞素材。
2. `#本週新聞 高潛力` 只顯示 `TopicPotential=高` 的素材。
3. `#本週新聞 詳細` 顯示較完整內容大綱、切角與節目潛力。
4. `#本週新聞 精簡` 只列分類、標題與來源。
5. `#本週新聞 24小時` 只看最近一天素材。
6. `#本週新聞 分類 <分類名>` 只看指定分類。
7. `#封存本週新聞` 的 prompt 改為「本週新聞週報索引」取向，保留代表性事件、人物、公司、平台、政策、作品名稱與主要脈絡。
8. `#help` 只顯示核心新聞工作流；`#help 進階` 收納詳細新聞檢視、`#懶人包`、`#節目話題分析`、`#統整話題`、`#畫重點` 與 `#封存本週話題`。
9. 更新 `#版本`、README、CURRENT_VERSION 與 changelog。

本版包含 NewsInbox 查詢與排版邏輯、新聞封存 prompt、help 分層與版本文件更新。

本版不刪除任何既有功能；不修改 Reader Layer；不導入 ByCrawl / Apify；不修改 NewsInbox schema；不修改 WeeklySummary schema；不改群組貼網址靜默收件流程；不新增 Node.js / npm / package.json / 自架伺服器架構；不寫入任何 secret。
...
## Explicitly Not Included in v1.12.1
...
* NewsInbox schema 或 Outline 欄位調整
...
## Suggested Smoke Tests for v1.12.1 Runtime

本版不新增或調整 Sheet schema；不需要手動執行 setup / migration 函式。若部署環境尚未套用 v1.12.0，仍需先確認 WeeklySummary 已具備 `ArchiveType`、`PeriodStart`、`PeriodEnd`、`SourceItemCount` 相容欄位。
...
* 執行 `#本週新聞 精簡`，確認只列分類、標題與來源。
* 執行 `#本週新聞 24小時`，確認只看最近一天素材。
* 執行 `#本週新聞 分類 科技與 AI`，確認只顯示該分類素材；可再換一個實際存在分類測試。
* 執行 `#help`，確認只顯示核心功能。
* 執行 `#help 進階`，確認顯示詳細新聞檢視、懶人包、節目話題分析、統整話題、畫重點與封存本週話題。
* 執行 `#封存本週新聞`，確認 WeeklySummary 新增 `ArchiveType=news` 的新聞封存。
* 檢查 `#封存本週新聞` 的摘要是否像週報索引，保留代表性事件、人物、公司、平台、政策、作品名稱與主要脈絡。
...
Last Confirmed Version at this Git ref: `v1.12.1 Weekly News Query & Help Focus Edition`
Previous stable baseline described in this file: `v1.12.0 Silent URL Status & News Archive Edition`
Last Confirmed Date: `2026-06-18`
Last Documentation Note: `#本週新聞` supports common query modes, `#封存本週新聞` uses a weekly news index prompt, and `#help` is focused on the core news workflow with advanced commands moved to `#help 進階`.
